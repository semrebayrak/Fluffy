...
import 'package:fluffy/Objects/walking.dart';
...
class ReserveWalk extends StatefulWidget {
  @override
  _ReserveWalkState createState() => _ReserveWalkState();
}

class _ReserveWalkState extends State<ReserveWalk> {
  bool reserved = false;
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < walk.reserved.length; i++) {
      absentDates.add(DateFormat("dd-MM-yyyy").parse(walk.reserved[i]));
    }
...
//      firstDayOfWeek: 4,
  ...
  Reserve(DateTime date, Walking walk) async {
    return await FirebaseAuthenticationService.setWalkReservation(date, walk)
        .then((s) => setState(() {
              reserved = s;
            }));
  }
